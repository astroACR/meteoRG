...
function getColor(value, variable) {
  ...
  // Darker palette for precipitation to ensure icon numeric text is readable
...
// Units (not used on icons; icons show numeric only)
...
legend.update = function () {
  ...
  if (currentVariable === "temperatura") {
    ...
    // fallback so grades is never undefined
    grades = [0];
...
// Marker clusters (colored by average of selected variable)
...
function fetchStations() {
  fetch("https://meteorg-service-93556534723.southamerica-west1.run.app/stations")
    ...
    .then(payload => {
      ...
      // Wrap plain array into FeatureCollection if needed
...
// --- Update FIRMS Layer Incrementally ---
...
function renderStations(data) {
  ...
  const geoJsonLayer = L.geoJSON(data, {
    pointToLayer: (feature, latlng) => {
      ...
      // Read values by selected variable
      let value;
      if (currentVariable === "temperatura") value = p.temperatura;
      if (currentVariable === "humedadRelativa") value = p.humedadRelativa;
      if (currentVariable === "aguaCaida24Horas") value = p.aguaCaida24Horas;
      ...
      // Wind spike parameters (offset origin a few px from center)
      ...
      const innerRadius = 15; // half of inner circle size (30px)
    ...
    onEachFeature: (feature, layer) => {
      ...
      // Footer includes institucion_sigla, altura, and momento
      layer.bindPopup(`
  ...
  stationLayer.clearLayers();       // Clear old markers
  markers.clearLayers();            // Clear cluster group

  markers.addLayer(geoJsonLayer);   // Add new markers to cluster group
  stationLayer.addLayer(markers);   // Add cluster group to stationLayer

  map.addLayer(stationLayer);       // Ensure it's visible

  // Conditional overlays (only when variable is selected + toggle on)
...
// Wire toggles to re-render when switched
